	...
	g.Describe("올바른 Webp이미지가 주어지면, 변환에 성공한다.", func() {
		...
		for _, item := range testImgs {
			...
			g.It(item.description, func() {
				runTestConvertToWebp(g, item.imgName, item.expectedWidth, item.expectedHeight)
			})
		}
	})
...
func runTestConvertToWebp(g *goblin.G, imageName string, exceptedWidth int, exceptedHeight int) {
	...
	err = ConvertToWebp(imgReader, 200, "test")

	resFile, err := os.Open(path.Join(config.GetSavePath(), "test.webp"))
	g.Assert(err).IsNil(err)
	defer resFile.Close()

	resImage, err := webp.Decode(resFile)
	g.Assert(err).IsNil(err)
